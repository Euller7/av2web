...
const Data = () => {
    ...
    const list = [
        ...
        useEffect(() => {
            const summedEntradas = list.reduce((acc, item) => acc + item.entrada, 0);
            setTotalEntradas(summedEntradas)

            const summedSaidas = list.reduce((acc, item) => acc + item.saida, 0);
            setTotalSaidas(summedSaidas)
    ...
    return(
        <Router>
            <Navbar />
            <Routes>
                <Route path="/" element={<Home totalEntradas={totalEntradas} totalSaidas={totalSaidas} total={total} />}></Route>
                <Route path="/transacoes" element={<Transacoes list={list}/>}></Route>
            </Routes>
            <Footer />
        </Router>
    )
